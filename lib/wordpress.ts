...
export async function getAllMagazineSlugs(): Promise<{ slug: string }[]> {
  if (!isConfigured) return []
  ...
  try {
    ...
    while (hasMore) {
      const response = await wordpressFetchPaginated<Magazine[]>(
        '/wp-json/wp/v2/h_magazine',
        { per_page: 100, page, _fields: 'slug' },
      )

      allSlugs.push(
        ...response.data.map((magazine) => ({ slug: magazine.slug })),
      )
      hasMore = page < response.headers.totalPages
      page++
    }
    ...
    console.warn(
      'WordPress unavailable, skipping static generation for magazines',
    )
    return []
  }
}
